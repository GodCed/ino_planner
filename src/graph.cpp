...
GridPose::GridPose(GridLocation location, int free_theta_start, int free_theta_length, int cost) : location_(location),
...
{
  return location_.costTo(pose.location_) + static_cast<double>(cost_) / 255.0 + (1.0 - thetaOverlapWith(pose) / 360.0);
}
    ...
    std::vector<GridPose> &poses)
{
  ...
  else if (cost < costmap_2d::INSCRIBED_INFLATED_OBSTACLE) // Maybe in a collision
  {
    ...
    // Orientation bin size ensure no footprint point move by more than one cell for each bin
    double r_cells = circumscribed_radius / costmap->getResolution();
    int m_bins = ceil(2.0 * M_PI / (1.0 / r_cells));
    int theta_bin = 360 / m_bins;

    for (int theta = 0; theta < 360; theta += theta_bin) // We check orientations...
    {
